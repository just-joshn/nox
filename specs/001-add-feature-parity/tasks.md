...
**Task-set status**: Complete executable workflow for the currently specified scope. T002–T007, T009–T013, and T062 close the discovery boundary; each discovered capability must receive observed evidence, an executable test-first implementation sequence, and verification coverage in this document before its owning checkpoint can close. Feature-family checkpoints are not substitutes for executable leaf tasks.
...
- T063–T078 and T081–T090 are family completion checkpoints; T079–T080 are cross-cutting audits. Checkpoint implementation clauses define the scope of T013-created leaf subtasks and do not authorize a whole-family source edit. Close a checkpoint only after every reachable leaf has observed evidence, an executable test-first implementation sequence, and verification coverage with passing scenarios, and gated leaves remain open.
...
- [ ] T006 Reconcile T002–T005 and T062 against every page in the current official documentation index, installed command tree, and restored-source candidates in `specs/001-add-feature-parity/reference/reconciliation.md`; list every unmatched source entry and split any broad item that can fail independently. Record the total reconciled leaf count, counts by feature family, gated-unverified count, unmatched-candidate count, and broad-item count still requiring decomposition.
...
- [ ] T011 Predeclare the required Pi visual matrix in `specs/001-add-feature-parity/reference/pi-visuals.md`. List the exact supported terminal widths, themes, platforms, and affected workflows. For each workflow, include applicable normal, denial, error, interruption, and recovery states, including prompts, menus, tool results, and errors. Define how a newly discovered width-, theme-, platform-, or state-specific difference expands the matrix. Record the matrix cell count and require every cell to have a Pi reference and comparison result. Any intentional visual departure requires a constitution amendment before implementation.
...
- [ ] T013 Continuously validate leaf-task coverage in `specs/001-add-feature-parity/reference/reconciliation.md`. Before a source slice begins, every affected leaf MUST have observed evidence, an executable test-first implementation sequence, and verification coverage. A leaf may use separate tasks, an explicitly ordered task set, or one task, but its mapping must require: (1) write and run the failing behavior test, (2) implement only after the expected failure is recorded, and (3) run and record verification. Keep T013 open globally until every reconciled leaf has valid task mappings, every mapped task ID exists, no family checkpoint substitutes for executable work, and the missing-mapping count is zero.

**Inventory dependency**: T002–T005 and T062 precede complete reconciliation through T006 and T007. Independently observed source slices may proceed only after their own T007 surface mapping, applicable T009–T011 evidence, and T013 leaf-task validation pass. Per-slice T013 validation and the completed T320 historical audit precede every additional source implementation task. Complete reconciliation remains mandatory before T046 and T053.
- [ ] T320 Audit every checked application implementation task against the current per-leaf entry gate before additional source work begins. In `specs/001-add-feature-parity/reference/validation.md`, record each task's leaf ID, evidence, observed failing-test result, implementation commit, verification result, Pi baseline impact, and security boundary. Uncheck and reopen any task whose required evidence cannot be demonstrated. Record the audited-task, passing, reopened, and unresolved counts; T320 passes only when the unresolved count is zero.
...
**Prerequisite for T016–T019**: `specs/001-add-feature-parity/reference/reconciliation.md` must map each affected US1 leaf to reference evidence, a failing test, exact source paths, and verification tasks. T013 must report zero missing mappings for these leaves. A domain heading or unobserved behavior does not satisfy this checkpoint.
...
- [ ] T016 [US1] Checkpoint — Confirm every context-discovery and prompt-handling leaf in the completed US1 slice has observed evidence, an executable test-first implementation sequence, and verification coverage; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T017 [US1] Checkpoint — Confirm every core file, search, and command-tool leaf in the completed US1 slice has separate availability, permission, execution, and failure contracts plus an executable test-first implementation sequence and verification coverage; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T018 [US1] Checkpoint — Confirm every affected interactive tool-activity, error, denial, and recovery state in the completed US1 slice has a leaf-specific integration task and corresponding Pi visual-matrix cell; record coverage in `specs/001-add-feature-parity/reference/pi-visuals.md`.
- [ ] T019 [US1] Checkpoint — Close the completed US1 slice only after all mapped leaf verification tasks pass, required Pi visual comparisons are recorded, and no reachable discrepancy for that slice remains open; record closure in `specs/001-add-feature-parity/reference/discrepancies.md` and `specs/001-add-feature-parity/reference/pi-visuals.md`.
...
- [ ] T063 [US1] Checkpoint — Verify every agent-execution leaf has an observed contract, an executable test-first implementation sequence, passing verification coverage, and Pi-state result; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T064 [US1] Checkpoint — Verify every built-in-tool leaf has an observed contract, an executable test-first implementation sequence, passing verification coverage, and applicable permission/error result; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T068 [US1] Checkpoint — Verify every terminal-interaction leaf has an observed contract, an executable test-first implementation sequence, passing verification coverage, and required Pi visual matrix result; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T081 [US1] Checkpoint — Verify every computer-use and browser-use leaf has an observed contract, an executable test-first implementation sequence, passing verification coverage, and permission/platform-gate result; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T086 [US1] Checkpoint — Verify every context-window, caching, image, large-input, and context-reduction leaf has an observed contract, an executable test-first implementation sequence, and passing verification coverage; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.

### Completed Work Record

> Historical record only. These checked tasks do not define execution order for pending work. T320 audits their compliance with the current per-leaf gate.
...
- [X] T186 [US1] Reconcile the synthetic Glob/Grep normal, no-match, invalid, and option traces into the US1 search inventory, scenarios, reconciliation, and gated register; correct nox source targets, preserve real-service and interactive gates, then commit locally in `specs/001-add-feature-parity/reference/inventory-core.md`, `reference/scenarios.md`, `reference/reconciliation.md`, and `reference/gated.md`.
...
- [ ] T023 [US2] Checkpoint — Verify every session identity, naming, continuation, resume, fork, compaction, and persistence leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T024 [US2] Checkpoint — Verify every permission-mode, rule-precedence, persistence, prompt, denial, unattended, credential-reuse, and redaction leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T025 [US2] Checkpoint — Verify every model, effort, fallback, context, and budget leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T026 [US2] Checkpoint — Verify every session and permission Pi-control leaf has observed evidence, an executable test-first implementation sequence, passing verification, and visual-matrix coverage mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T027 [US2] Checkpoint — Close background-session lifecycle only after T281–T319 pass, every T281 leaf has matched reference evidence, and all related discrepancies are closed in `specs/001-add-feature-parity/reference/discrepancies.md`.
...
- [ ] T285 [US2] Observe the T281-assigned background-log-read leaf in an isolated fixture; record output ordering, redaction, missing-session behavior, and side effects in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T286 [US2] Observe the T281-assigned background-stop leaf in an isolated fixture; record state transition, exit behavior, repeated-stop error, and persisted side effects in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T287 [US2] Observe the T281-assigned background-restart leaf in an isolated fixture; record identity reuse/replacement, state transition, failure behavior, and persisted side effects in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T288 [US2] Observe the T281-assigned background-remove leaf in an isolated fixture; record cleanup, missing-session behavior, active-session handling, and persisted side effects in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T289 [US2] Capture matched reference observations for US2-SESSION-001 through US2-SESSION-006 under `specs/001-add-feature-parity/reference/observations/` and record normalized contracts in `specs/001-add-feature-parity/reference/scenarios.md`.
- [ ] T290 [US2] Compare the T020 nox baseline against T289, record each result and discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`, and mark only passing leaves verified.
- [ ] T291 [US2] Observe the T281-assigned background-unexpected-exit leaf in an isolated fixture; record terminal state, exit metadata, logs, cleanup, and redacted failure output in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T292 [US2] Observe the T281-assigned background-recovery leaf after T291; record restart/attach options, identity and history behavior, resulting state, and failure paths in `specs/001-add-feature-parity/reference/scenarios.md` and `specs/001-add-feature-parity/reference/observations/`.
- [ ] T293 [US2] Write a failing background-launch behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T282, covering normal and invalid launch without provider credentials or network access.
- [ ] T294 [US2] Implement only the observed background-launch state transition in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, validating input before process or filesystem side effects.
- [ ] T295 [US2] Verify the background-launch leaf with focused tests and record its normalized comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T296 [US2] Write a failing background-list behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T283, covering populated, empty, and error states.
- [ ] T297 [US2] Implement only the observed background-list behavior in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`.
- [ ] T298 [US2] Verify the background-list leaf with focused tests and record its normalized comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T299 [US2] Write a failing background-attach behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T284, covering attach, disconnect, and missing-session states.
- [ ] T300 [US2] Implement only the observed background-attach behavior in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, validating the session identifier before attachment.
- [ ] T301 [US2] Verify the background-attach leaf with focused tests and affected Pi states; record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T302 [US2] Write a failing background-log-read behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T285, covering ordering, redaction, and missing sessions.
- [ ] T303 [US2] Implement only the observed background-log-read behavior in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, preserving required redaction.
- [ ] T304 [US2] Verify the background-log-read leaf with focused tests and record its comparison, redaction result, or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T305 [US2] Write a failing background-stop behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T286, covering active, stopped, and missing sessions.
- [ ] T306 [US2] Implement only the observed background-stop transition in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, validating authorization and identity before signaling a process.
- [ ] T307 [US2] Verify the background-stop leaf with focused tests and record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T308 [US2] Write a failing background-restart behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T287, covering normal and failed restart.
- [ ] T309 [US2] Implement only the observed background-restart transition in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, preserving the observed identity/history contract.
- [ ] T310 [US2] Verify the background-restart leaf with focused tests and record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T311 [US2] Write a failing background-remove behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T288, covering completed, active, and missing sessions.
- [ ] T312 [US2] Implement only the observed background-remove transition in `packages/coding-agent/src/core/background-session.ts` and its nox-native CLI entry in `packages/coding-agent/src/cli.ts`, validating lifecycle state before cleanup.
- [ ] T313 [US2] Verify the background-remove leaf with focused tests and record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T314 [US2] Write a failing unexpected-exit behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T291, covering exit metadata, logs, and terminal state.
- [ ] T315 [US2] Implement only the observed unexpected-exit transition in `packages/coding-agent/src/core/background-session.ts`, preserving logs and redacting failure output.
- [ ] T316 [US2] Verify the unexpected-exit leaf with focused tests and record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
- [ ] T317 [US2] Write a failing background-recovery behavior test in `packages/coding-agent/test/suite/parity-session-lifecycle.test.ts` from T292, covering successful and failed recovery after unexpected exit.
- [ ] T318 [US2] Implement only the observed background-recovery transition in `packages/coding-agent/src/core/background-session.ts` and affected nox-native controls in `packages/coding-agent/src/cli.ts`.
- [ ] T319 [US2] Verify the background-recovery leaf with focused and neighboring session/CLI tests, `npm run check`, and affected T093 Pi regression cells; record its comparison or discrepancy in `specs/001-add-feature-parity/reference/validation.md` and `specs/001-add-feature-parity/reference/discrepancies.md`.
...
- [ ] T065 [US2] Checkpoint — Verify every model/provider-control leaf has an observed contract, an executable test-first implementation sequence, and passing normal, unavailable, rate-limit, and restart verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T066 [US2] Checkpoint — Verify every permission/security leaf has an observed contract, an executable test-first implementation sequence, and passing allow, deny, policy, restart, and disclosure verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T067 [US2] Checkpoint — Verify every session/context leaf has an observed contract, an executable test-first implementation sequence, and passing persistence, crash, repository-change, and context-limit verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T082 [US2] Checkpoint — Verify every goal, agent-view, cross-session-message, and dynamic-workflow leaf has an observed contract, an executable test-first implementation sequence, and passing coordination/recovery verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T087 [US2] Checkpoint — Verify every scheduled-prompt, reminder, repeated-prompt, and goal-continuation leaf has an observed contract, an executable test-first implementation sequence, and passing time-controlled verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
...
- [ ] T031 [US3] Checkpoint — Verify every settings validation, scope-precedence, and nox-native location leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T032 [US3] Checkpoint — Verify every instruction, rule, and memory discovery/precedence leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T033 [US3] Checkpoint — Verify every skill and custom-command discovery, invocation, argument, and failure leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T034 [US3] Checkpoint — Verify every agent isolation, tool-selection, lifecycle, and result leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T035 [US3] Checkpoint — Verify every hook event, ordering, input/output, blocking, and failure leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T036 [US3] Checkpoint — Verify every external-connection discovery, authorization, resource, prompt, tool, exposure, and redaction leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T037 [US3] Checkpoint — Verify every plugin discovery, enablement, namespace, installation, and update leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
...
- [ ] T069 [US3] Checkpoint — Verify every settings, instruction, rules, and memory leaf has an observed contract, an executable test-first implementation sequence, and passing conflict, invalid-value, restart, and isolation verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T070 [US3] Checkpoint — Verify every skill and command leaf has an observed contract, an executable test-first implementation sequence, and passing duplicate, disabled, error, and precedence verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T071 [US3] Checkpoint — Verify every hook leaf has an observed contract, an executable test-first implementation sequence, and passing ordering, cancellation, invalid-output, restart, and disclosure verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T072 [US3] Checkpoint — Verify every agent, subagent, team, task, message, and delegation leaf has an observed contract, an executable test-first implementation sequence, and passing failure/cancellation/concurrency verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T073 [US3] Checkpoint — Verify every plugin, marketplace, and external-connection leaf has an observed contract, an executable test-first implementation sequence, and passing scope, offline, permission, redaction, and lifecycle verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T084 [US3] Checkpoint — Verify every plugin-evaluation leaf has an observed contract, an executable test-first implementation sequence, and passing repeatability and failure verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T088 [US3] Checkpoint — Verify every extension-diagnostic and debugging leaf has an observed contract, an executable test-first implementation sequence, and passing precedence, disabled, invalid-setting, and connection-failure verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
...
- [ ] T041 [US4] Checkpoint — Verify every print format, structured-output, stream, partial-event, RPC, and exit leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T042 [US4] Checkpoint — Verify every worktree isolation, naming, lifecycle, and cleanup leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T043 [US4] Checkpoint — Verify every remote and desktop/web/mobile/editor/browser/chat/CI leaf has a nox control, an observed interaction/data-exposure contract, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`; retain inaccessible leaves as `gated-unverified`.
- [ ] T044 [US4] Checkpoint — Verify every authentication, diagnostic, import, project-state, update, external-management, and hosted-review command leaf has observed evidence, an executable test-first implementation sequence, and passing verification mapped in `specs/001-add-feature-parity/reference/reconciliation.md`.
...
- [ ] T074 [US4] Checkpoint — Verify every noninteractive and programmatic leaf has an observed contract, an executable test-first implementation sequence, and passing malformed-input, backpressure, interruption, and failure verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T075 [US4] Checkpoint — Verify every CLI and administration leaf has an observed contract, an executable test-first implementation sequence, and passing help, default, conflict, exit, and state-change verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T076 [US4] Checkpoint — Verify every worktree, source-control, automation, scheduling, hosted-review, and notification leaf has an observed contract, an executable test-first implementation sequence, and passing lifecycle verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T077 [US4] Checkpoint — Verify every remote and platform-surface leaf has an observed contract, an executable test-first implementation sequence, and passing entry, interaction, disclosure, failure, disconnect, and resume verification; record gated leaves and missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T078 [US4] Checkpoint — Verify every documented terminal, editor, desktop, web, mobile, browser, chat, CI, and SDK workflow maps to a nox control and explicit leaf task set, with applicable Pi visual results recorded in `specs/001-add-feature-parity/reference/surface-map.md`.
- [ ] T083 [US4] Checkpoint — Verify every hosted-review and security-workflow leaf has an observed contract, an executable test-first implementation sequence, and passing permission, disclosure, provider, and gated-case verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T085 [US4] Checkpoint — Verify every artifact, sharing, deep-link, and cross-surface-access leaf has an observed contract, an executable test-first implementation sequence, and passing ownership, denial, expiration, and disconnect verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T089 [US4] Checkpoint — Verify every desktop scheduling, dispatch, preview, visual-diff, and editor/simulator leaf has an observed contract, an executable test-first implementation sequence, and passing handoff, permission, failure, and service-gate verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
- [ ] T090 [US4] Checkpoint — Verify every deep-link, channel-event, and external-trigger leaf has an observed contract, an executable test-first implementation sequence, and passing ordering, replay, disconnect, and authorization verification; record missing mappings in `specs/001-add-feature-parity/reference/reconciliation.md`.
...
Reference discovery (T001–T005 and T062)
    → Reconciliation and surfaces (T006 → T007)
        → Per-feature evidence (T009–T011, with T008/T012 schemas, T054–T061 core observations, T091 coverage baseline, and T093 Pi functional baseline)
            → Zero-missing leaf task validation (T013)
...
- Per-slice T013 validation and the completed T320 historical audit precede every additional source implementation task. T320 may validate historical tasks using any explicitly ordered test-first form accepted by T013. T062 precedes T006. T079–T080 precede T046. T046 can expose missing leaf tasks; add them in the owning story phase and complete them before T051–T052. T093 precedes every source slice that can affect a Pi workflow; T091 and T093 precede T049. T092 precedes T053. T053 follows all validation.
...
- **US1**: T014 and T015 may run concurrently; each executable US1 leaf task requires T013 to report zero missing mappings for that leaf. T016–T019 summarize the completed core slice and do not authorize future source work.
- **US2**: T020 is a nox-only baseline; T289–T290 provide its matched reference comparison. Background lifecycle follows T281 → per-leaf observation → per-leaf failing test → per-leaf implementation → per-leaf verification → T027. T021–T022 require observed leaf IDs before execution; T023–T026 remain checkpoints until T013 creates their leaf tasks.
- **US3**: T029 and T030 may run concurrently after their leaf evidence exists; T031–T037 are checkpoints until T013 validates executable leaf work.
- **US4**: T039 and T040 may run concurrently after their leaf evidence exists; T041–T044 are checkpoints until T013 validates executable leaf work.
