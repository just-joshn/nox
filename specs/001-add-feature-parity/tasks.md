...
These tasks are listed after the original 53 IDs to preserve stable task numbering. They precede T013 for the selected US1 leaves and do not authorize source implementation without observed normal, failure, and relevant interaction contracts. Only a read request was probed while unauthenticated, as recorded in `specs/001-add-feature-parity/reference/observations/us1-unauthenticated.txt`; the other leaves were not probed.
