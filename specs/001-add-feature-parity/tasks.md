...
- [ ] T110 [US4] Add explicit prompt-file arguments and validation in `packages/coding-agent/src/cli/args.ts` and `packages/coding-agent/src/main.ts`, retaining the existing resource loader's file-source semantics. Reject conflicts and missing files before session/model work; preserve inline prompt behavior.
- [ ] T111 [US4] Verify the focused prompt-file and nearby CLI tests, `npm run check`, and the synthetic CLI-558 composition fixture; record any mismatch in `reference/validation.md` and `reference/discrepancies.md`, then commit locally.
