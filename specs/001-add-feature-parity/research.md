...
- **Decision**: For each application leaf, state its observed contract and simplest viable change, run a focused failing behavior test, implement, then run relevant unit, integration, and end-to-end checks. Add an offline aggregate coverage report across every production workspace package and session backend shipped or imported by nox before enforcing the project-wide 80% gate; count each owned source file once and document exclusions. Require aggregate lines, statements, functions, and branches each to reach 80%; zero-denominator or unmeasured metrics fail. Capture unrelated Pi functional baselines before source work and compare affected workflows after each slice. The repository currently exposes package tests and one agent harness coverage command, not a project-wide coverage command. Keep changes limited to the owning code path with immutable replacement state.
