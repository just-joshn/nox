...
7. Before each code change, run a focused behavior test and confirm that it fails for the expected reason. Make the smallest change that passes it, then run relevant unit, integration, and end-to-end checks, `npm run check`, and a project coverage report. Application work cannot be called complete below 80% coverage. Review the diff for credentials, input validation, authorization, applicable injection and request-forgery defenses, rate limits, and data leaks. Use isolated fixtures and no paid provider calls for routine tests.
