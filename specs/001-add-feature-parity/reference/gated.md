...
| TOOL-003 | Sandboxed normal and no-match Grep calls returned non-error results, but exact result formatting, invalid queries, permission behavior, and real-service use remain unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Grep calls; compare redacted result formatting, invalid query, and permission behavior |
