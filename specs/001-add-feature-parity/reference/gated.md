...
| TOOL-003 | Sandboxed normal-startup and bare-mode catalog omission observed, but explicit Grep enabling and tool behavior remain unobserved | Same disposable network sandbox and localhost Messages endpoint; compare `--tools Grep`, normal/no-match/invalid patterns, and permission behavior |
