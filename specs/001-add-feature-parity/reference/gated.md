...
| TOOL-002 | Sandboxed normal-startup and bare-mode catalog omission observed, but explicit Glob enabling and tool behavior remain unobserved | Same disposable network sandbox and localhost Messages endpoint; compare `--tools Glob`, normal/no-match/invalid patterns, and permission behavior |
