...
| TOOL-003 | Sandboxed normal Grep call found a synthetic fixture, but exact result formatting, no-match/invalid queries, permission behavior, and real-service use remain unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Grep calls; compare redacted normal/no-match/invalid result shape and permission behavior |
