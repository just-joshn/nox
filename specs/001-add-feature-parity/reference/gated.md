...
| TOOL-002 | Sandboxed normal and no-match Glob calls returned non-error results, but exact result formatting, invalid patterns, permission behavior, and real-service use remain unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Glob calls; compare redacted result formatting, invalid pattern, and permission behavior |
