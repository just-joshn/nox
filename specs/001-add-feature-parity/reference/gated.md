...
| TOOL-002 | Sandboxed normal-startup omission and explicit `--tools Glob` exposure observed, but Glob pattern behavior remains unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Glob tool calls; compare normal/no-match/invalid patterns and permission behavior |
