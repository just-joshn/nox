...
| TOOL-002 | Sandboxed normal Glob call found a synthetic fixture, but exact result formatting, no-match/invalid patterns, permission behavior, and real-service use remain unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Glob calls; compare redacted normal/no-match/invalid result shape and permission behavior |
| TOOL-003 | Sandboxed normal-startup omission and explicit `--tools Grep` exposure observed, but Grep query behavior remains unobserved | Same disposable network sandbox and localhost Messages endpoint with synthetic Grep tool calls; compare normal/no-match/invalid patterns and permission behavior |
