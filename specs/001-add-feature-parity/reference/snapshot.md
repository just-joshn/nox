...
This snapshot captures discovery evidence, not behavioral observations for every feature. No provider call, subscription flow, external-service action, reference binary inspection, or interactive session was performed. Official documentation is live and may describe features newer than 2.1.270; version-gated claims require separate confirmation. The inventory files distinguish `observed-help`, `documented`, and `gated-unverified` so they cannot be mistaken for verified parity.
