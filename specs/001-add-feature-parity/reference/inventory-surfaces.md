...
| SUR-PLAT-018 | Change permission mode from a mobile cloud or Remote Control session | Cloud offers Accept edits/Plan/Auto; local remote offers Manual/Accept edits/Plan; bypass is absent | Disallowed mode or stale state must not change permission policy | Mobile account and session-kind gate; nox remote mode selector |
