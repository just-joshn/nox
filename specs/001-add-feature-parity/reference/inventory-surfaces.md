...
| SUR-PLAT-013 | Open the mobile Code tab and select a repository and branch, then submit a task | Cloud session persists across devices and continues when the phone disconnects | Missing Code tab, repository authorization failure, stale progress | Mobile cloud account gate; nox cloud session list |
| SUR-PLAT-014 | Open an active cloud session on mobile; answer a question or steer it | The existing hosted session receives the response and updates progress | Offline reply, session ended, wrong-session delivery | Mobile cloud account gate; nox cloud reply control |
| SUR-PLAT-015 | Select a local Remote Control session in the mobile Code tab, by list or QR pairing | Phone controls the local conversation while files and tools remain on the computer | Host sleeps or disconnects; reconnect on wake | Mobile plus remote-control gate; nox remote session attach |
| SUR-PLAT-016 | Attach a photo in a mobile Remote Control message | Local session receives the image and saves a copy under its upload area with a path in context | Upload failure, path collision, revoked session | Mobile plus remote-control gate; nox remote image attachment |
| SUR-PLAT-017 | Attach a non-photo file in a mobile Remote Control message | File downloads to the local machine and enters the turn as a file reference | Download denial, unsupported file, disconnect | Mobile plus remote-control gate; nox remote file attachment |
| SUR-PLAT-018 | Change permission mode in a mobile cloud session | Cloud mode selector offers Accept edits, Plan, and Auto but not bypass | Disallowed mode or stale state must not change cloud permission policy | Mobile cloud account gate; nox cloud mode selector |
| SUR-PLAT-019 | Ask for a completion or decision push while Remote Control is active | Notification reaches the paired phone; Dispatch uses its own task notification path | Notifications disabled, device offline, delivery failure | Mobile device and notification gate; nox notification bridge |
| SUR-PLAT-020 | Run `/mobile` in a local terminal session | Mobile app download QR code appears; scanning opens installation path | Terminal cannot render code, unsupported device, stale link | Local terminal plus mobile device gate; nox mobile onboarding command |
| SUR-PLAT-021 | Change permission mode in a mobile Remote Control session | Local remote selector offers Manual, Accept edits, and Plan but not Auto or bypass | Disallowed mode or stale state must not change local permission policy | Mobile plus remote-control gate; nox remote mode selector |
| SUR-PLAT-022 | Run `/ios` in a local terminal session | iOS app download QR code appears | Terminal cannot render code, unsupported device, stale link | Local terminal plus iOS device gate; nox iOS onboarding command |
| SUR-PLAT-023 | Run `/android` in a local terminal session | Android app download QR code appears | Terminal cannot render code, unsupported device, stale link | Local terminal plus Android device gate; nox Android onboarding command |
