...
SEARCH_MODES = frozenset({
    ...
    "grep-content-tie-reversed", "grep-content-page",
})
...
NON_TXT_MATCH_MODES = {"grep-type-filter", "grep-content-page"}
...
def prepare_workspace(root: str, mode: str) -> tuple[Path, Path, Path | None]:
    ...
    if mode in {"glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page"}:
        (workspace / "second.txt").write_text(FIXTURE_CONTENT)
    if mode in {"grep-content-multiple", "grep-content-page"}:
...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        state = ProbeState(read_path=read_path,
                           ...
                           head_limit=1 if mode in {"grep-head-limit", "grep-offset", "grep-head-exact", "grep-offset-end", "grep-offset-zero", "grep-content-page"} else None,
        ...
        try:
            ...
            summary = {
                ...
                "second_unchanged": (workspace / "second.txt").read_text() == (FIXTURE_CONTENT if mode in {"glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page"} else "alpha\n") if mode in {"grep-count-multiple", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page"} else None,
                "content_mtime_unchanged": (fixture.stat().st_mtime == 1_600_000_000 and
                                            (workspace / "second.txt").stat().st_mtime == 1_700_000_000)
                                           if mode in {"grep-content-multiple", "grep-content-page"} else None,
    ...
    if mode in SEARCH_MODES:
        requests = summary.get("requests", [])
        matched = ((mode not in {"grep-count-multiple", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page"} or summary.get("second_unchanged") is True)
                   and (mode not in {"grep-content-multiple", "grep-content-page"} or summary.get("content_mtime_unchanged") is True)
