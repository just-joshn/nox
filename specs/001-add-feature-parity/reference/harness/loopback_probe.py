...
def prepare_workspace(root: str, mode: str) -> tuple[Path, Path, Path | None]:
    ...
    if mode in {"glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed"}:
        (workspace / "second.txt").write_text(FIXTURE_CONTENT)
    if mode == "grep-content-multiple":
...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        try:
            ...
            summary = {
                ...
                "second_unchanged": (workspace / "second.txt").read_text() == (FIXTURE_CONTENT if mode in {"glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed"} else "alpha\n") if mode in {"grep-count-multiple", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed"} else None,
                "content_mtime_unchanged": (fixture.stat().st_mtime == 1_600_000_000 and
                                            (workspace / "second.txt").stat().st_mtime == 1_700_000_000)
                                           if mode == "grep-content-multiple" else None,
    ...
    if mode in SEARCH_MODES:
        requests = summary.get("requests", [])
        matched = ((mode not in {"grep-count-multiple", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed"} or summary.get("second_unchanged") is True)
                   and (mode != "grep-content-multiple" or summary.get("content_mtime_unchanged") is True)
