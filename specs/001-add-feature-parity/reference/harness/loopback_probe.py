...
NO_MATCH_MODES = {"glob-no-match", "grep-no-match", "grep-count-no-match", "grep-offset-end", "grep-multiline-no-match", "grep-files-offset-end", "grep-files-no-match", "grep-content-no-match", "grep-content-no-match-page", "grep-content-no-match-offset", "grep-files-no-match-offset"}
...
def prepare_workspace(root: str, mode: str) -> tuple[Path, Path, Path | None]:
    ...
    if mode in {"grep-count-multiple", "grep-count-offset"}:
...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        state = ProbeState(read_path=read_path,
                           ...
                           head_limit=1 if mode in {"grep-head-limit", "grep-offset", "grep-head-exact", "grep-offset-end", "grep-offset-zero", "grep-content-page", "grep-files-page", "grep-files-exact", "grep-files-offset", "grep-files-offset-end", "grep-content-no-match-page", "grep-content-no-match-offset", "grep-files-no-match-offset", "grep-count-offset"} else None,
                           offset=1 if mode in {"grep-offset", "grep-files-offset", "grep-content-no-match-offset", "grep-files-no-match-offset", "grep-count-offset"} else 2 if mode in {"grep-offset-end", "grep-files-offset-end"} else 0 if mode == "grep-offset-zero" else None,
    ...
    if mode in SEARCH_MODES:
        requests = summary.get("requests", [])
        matched = ((mode not in {"grep-count-multiple", "grep-count-offset", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page", "grep-files-page", "grep-files-offset", "grep-files-offset-end"} or summary.get("second_unchanged") is True)
