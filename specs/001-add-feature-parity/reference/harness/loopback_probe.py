...
SEARCH_MODES = frozenset({
    ...
    "grep-head-zero",
    "grep-head-fraction",
    "grep-offset-fraction",
    "grep-context-negative",
    ...
    "grep-file-default",
    "grep-file-no-match",
    "grep-file-content-no-match",
    "grep-file-count-no-match",
    "grep-path-missing",
    "glob-path-missing",
    "glob-file-path",
})
...
NO_MATCH_MODES = {"glob-no-match", "grep-no-match", "grep-count-no-match", "grep-offset-end", "grep-multiline-no-match", "grep-files-offset-end", "grep-files-no-match", "grep-content-no-match", "grep-content-no-match-page", "grep-content-no-match-offset", "grep-files-no-match-offset", "grep-count-no-match-offset", "grep-count-no-match-page", "grep-file-no-match", "grep-file-content-no-match", "grep-file-count-no-match"}
NON_TXT_MATCH_MODES = {"grep-type-filter", "grep-content-page", "grep-files-page", "grep-count-page", "grep-count-offset-end", "grep-count-mtime", "grep-count-recreate", "grep-pattern-empty", "grep-file-content"}
INVALID_MODES = {"glob-invalid", "grep-invalid", "grep-type-invalid", "grep-output-invalid", "grep-head-negative", "grep-offset-negative", "grep-head-fraction", "grep-offset-fraction", "grep-context-negative", "grep-context-alias-negative", "grep-after-negative", "grep-before-negative", "grep-path-missing", "glob-path-missing", "glob-file-path"}
...
def message_response(model: str, kind: str, read_path: str, tool_name: str = "Read", tool_pattern: str | None = None,
    ...
    if kind == "tool":
        ...
        if context_side == "-A-negative": tool_input = {**tool_input, "-A": -1}
        if context_side == "-B-negative": tool_input = {**tool_input, "-B": -1}
...
def summarize_search_result(result: dict, tool_name: str = "") -> dict:
    ...
    return {"is_error": bool(result.get("is_error")), "fixture_name_present": "fixture.txt" in text,
            "result_format": "fixture_match" if text == expected.get(tool_name) else
                             ...
                             "direct_file_content" if text == "1:alpha" and tool_name == "Grep" else
...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        state = ProbeState(read_path=read_path,
                           ...
                           tool_pattern="absent-*.zzz" if mode == "glob-no-match" else "absent-sentinel" if mode in {"grep-no-match", "grep-count-no-match", "grep-multiline-no-match", "grep-files-no-match", "grep-content-no-match", "grep-content-no-match-page", "grep-content-no-match-offset", "grep-files-no-match-offset", "grep-count-no-match-offset", "grep-count-no-match-page", "grep-file-no-match", "grep-file-content-no-match", "grep-file-count-no-match"} else "[" if mode in {"glob-invalid", "grep-invalid"} else "" if mode == "grep-pattern-empty" else "ALPHA" if mode == "grep-ignore-case" else "alpha\nbeta" if mode in {"grep-multiline", "grep-multiline-files", "grep-multiline-explicit-files", "grep-multiline-count"} else "**/*.txt" if mode in {"glob-recursive", "glob-git-metadata"} else None,
                           tool_path="nested" if mode in {"glob-path", "grep-path", "grep-path-files"} else "fixture.txt" if mode in {"grep-pattern-empty", "grep-file-content", "grep-file-files", "grep-file-count", "grep-file-default", "grep-file-no-match", "grep-file-content-no-match", "grep-file-count-no-match", "glob-file-path"} else "absent-dir" if mode in {"grep-path-missing", "glob-path-missing"} else None,
                           output_mode="files_with_matches" if mode in {"grep-files-mode", "grep-path-files", "grep-multiline-explicit-files", "grep-files-page", "grep-files-exact", "grep-files-offset", "grep-files-offset-end", "grep-files-no-match", "grep-files-no-match-offset", "grep-file-files"} else "content" if mode in {"grep-content-mode", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page", "grep-no-line-number", "grep-only-matching", "grep-context", "grep-context-alias", "grep-after-context", "grep-before-context", "grep-head-limit", "grep-offset", "grep-head-exact", "grep-offset-end", "grep-offset-zero", "grep-multiline", "grep-multiline-no-match", "grep-content-no-match", "grep-content-no-match-page", "grep-content-no-match-offset", "grep-head-negative", "grep-offset-negative", "grep-head-zero", "grep-head-fraction", "grep-offset-fraction", "grep-context-negative", "grep-context-alias-negative", "grep-after-negative", "grep-before-negative", "grep-pattern-empty", "grep-file-content", "grep-file-content-no-match", "grep-path-missing"} else "count" if mode in {"grep-count-mode", "grep-count-multiple", "grep-count-no-match", "grep-count-limit", "grep-count-same-line", "grep-multiline-count", "grep-count-offset", "grep-count-page", "grep-count-exact", "grep-count-offset-end", "grep-count-no-match-offset", "grep-count-no-match-page", "grep-count-offset-zero", "grep-count-mtime", "grep-count-recreate", "grep-file-count", "grep-file-count-no-match"} else "bogus" if mode == "grep-output-invalid" else None,
    ...
    if mode in SEARCH_MODES:
        ...
        matched = ((mode not in {"grep-count-multiple", "grep-count-offset", "grep-count-page", "grep-count-mtime", "grep-count-recreate", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page", "grep-files-page", "grep-files-offset", "grep-files-offset-end"} or summary.get("second_unchanged") is True)
                   ...
                   and requests[2]["tool_results"][0].get("fixture_name_present") is (mode in FIXTURE_MATCH_MODES or mode == "glob-file-path")
