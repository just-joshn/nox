...
NO_MATCH_MODES = {"glob-no-match", "grep-no-match", "grep-count-no-match", "grep-offset-end", "grep-multiline-no-match", "grep-files-offset-end", "grep-files-no-match", "grep-content-no-match", "grep-content-no-match-page"}
NON_TXT_MATCH_MODES = {"grep-type-filter", "grep-content-page", "grep-files-page"}
...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        state = ProbeState(read_path=read_path,
                           ...
                           tool_pattern="absent-*.zzz" if mode == "glob-no-match" else "absent-sentinel" if mode in {"grep-no-match", "grep-count-no-match", "grep-multiline-no-match", "grep-files-no-match", "grep-content-no-match", "grep-content-no-match-page"} else "[" if mode in {"glob-invalid", "grep-invalid"} else "ALPHA" if mode == "grep-ignore-case" else "alpha\nbeta" if mode in {"grep-multiline", "grep-multiline-files", "grep-multiline-explicit-files", "grep-multiline-count"} else "**/*.txt" if mode in {"glob-recursive", "glob-git-metadata"} else None,
                           tool_path="nested" if mode in {"glob-path", "grep-path", "grep-path-files"} else None,
                           output_mode="files_with_matches" if mode in {"grep-files-mode", "grep-path-files", "grep-multiline-explicit-files", "grep-files-page", "grep-files-exact", "grep-files-offset", "grep-files-offset-end", "grep-files-no-match"} else "content" if mode in {"grep-content-mode", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page", "grep-no-line-number", "grep-only-matching", "grep-context", "grep-context-alias", "grep-after-context", "grep-before-context", "grep-head-limit", "grep-offset", "grep-head-exact", "grep-offset-end", "grep-offset-zero", "grep-multiline", "grep-multiline-no-match", "grep-content-no-match", "grep-content-no-match-page"} else "count" if mode in {"grep-count-mode", "grep-count-multiple", "grep-count-no-match", "grep-count-limit", "grep-count-same-line", "grep-multiline-count"} else None,
                           ...
                           head_limit=1 if mode in {"grep-head-limit", "grep-offset", "grep-head-exact", "grep-offset-end", "grep-offset-zero", "grep-content-page", "grep-files-page", "grep-files-exact", "grep-files-offset", "grep-files-offset-end", "grep-content-no-match-page"} else None,
                           offset=1 if mode in {"grep-offset", "grep-files-offset"} else 2 if mode in {"grep-offset-end", "grep-files-offset-end"} else 0 if mode == "grep-offset-zero" else None,
    ...
    if mode in SEARCH_MODES:
        requests = summary.get("requests", [])
        matched = ((mode not in {"grep-count-multiple", "glob-two-files", "grep-content-multiple", "grep-content-tie", "grep-content-tie-reversed", "grep-content-page", "grep-files-page", "grep-files-offset", "grep-files-offset-end"} or summary.get("second_unchanged") is True)
