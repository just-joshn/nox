...
def main(executable: str, mode: str = "normal") -> int:
    with tempfile.TemporaryDirectory(prefix="nox-loopback-read-") as root:
        ...
        state = ProbeState(read_path=read_path,
                           ...
                           tool_path="nested" if mode in {"glob-path", "grep-path", "grep-path-files"} else "fixture.txt" if mode in {"grep-pattern-empty", "grep-file-content", "grep-file-files", "grep-file-count", "grep-file-default"} else None,
