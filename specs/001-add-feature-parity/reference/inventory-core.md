...
| TOOL-002 | `Glob`: enumerate paths by pattern | No in-scope; absent by default on macOS/Linux/WSL | Documented; local availability unobserved |
| TOOL-003 | `Grep`: search file content by pattern | No in-scope; absent by default on macOS/Linux/WSL | Documented; local availability unobserved |
...
|----|--------------------------|--------------------|------------|----------------|
| US1-READ-001 | Read an in-scope text file and report result | Installed `--tools` help names `Read`; synthetic loopback [normal](observations/read-loopback-2026-09-14.json) and [missing-path](observations/read-missing-loopback-2026-09-14.json) traces | `packages/coding-agent/src/utils/tools-manager.ts` | Partial normal and error result-shape observation; exact error/permission and real-service cases `gated-unverified` |
