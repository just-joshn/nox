...
| TOOL-002 | `Glob`: enumerate paths by pattern | No in-scope; absent by default on macOS/Linux/WSL | [Default](observations/tool-default-catalog-loopback-2026-09-14.json) and [bare](observations/tool-bare-catalog-loopback-2026-09-14.json) requests omit Glob; [explicit catalog](observations/tool-glob-enabled-loopback-2026-09-14.json) includes it; [normal](observations/tool-glob-normal-loopback-2026-09-14.json) and [no-match](observations/tool-glob-no-match-loopback-2026-09-14.json) calls are non-errors, [invalid bracket](observations/tool-glob-bracket-loopback-2026-09-14.json) is a tool error; exact wording unverified |
| TOOL-003 | `Grep`: search file content by pattern | No in-scope; absent by default on macOS/Linux/WSL | [Default](observations/tool-default-catalog-loopback-2026-09-14.json) and [bare](observations/tool-bare-catalog-loopback-2026-09-14.json) requests omit Grep; [explicit catalog](observations/tool-grep-enabled-loopback-2026-09-14.json) includes it; [normal](observations/tool-grep-normal-loopback-2026-09-14.json) and [no-match](observations/tool-grep-no-match-loopback-2026-09-14.json) calls return non-error results; exact result/invalid-query behavior unverified |
...
These IDs split the initial coding journey for evidence collection. The installed CLI help names `Read`, `Edit`, and `Bash`; the [official tools reference](https://code.claude.com/docs/en/tools-reference) also documents `Grep` and `Glob`, but their availability in this installed configuration has not been observed. An earlier sandboxed print-mode read request stopped at authentication before tool dispatch ([raw observation](observations/us1-unauthenticated.txt)); a later status check outside the sandbox reported authentication available. The user reports that the weekly usage cap is reached, so no model or tool probe followed. Neither condition demonstrates a failure of an individual tool. No row below has a normal tool trace or passing nox comparison.
