...
| Read, search, edit, write, and command tools | Request and result card for each tool | Permission prompt and denied result | Missing file, bad input, timeout, command failure | Corrected tool call or retry | Read card request/success/error ANSI captured; remaining states pending |
