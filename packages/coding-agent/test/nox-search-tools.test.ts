...
describe("explicit nox parity search tools", () => {
	...
	it("Glob rejects a file path as search directory", async () => {
		const tool = createAllToolDefinitions(cwd).Glob;
		await expect(
			tool.execute("call-1", { pattern: "*.txt", path: "fixture.txt" }, undefined, undefined, {} as never),
		).rejects.toThrow();
	});
	...
	it("Grep direct-file content mode reports no matches", async () => {
		const tool = createAllToolDefinitions(cwd).Grep;
		const result = await tool.execute(
			"call-1",
			{ pattern: "absent-sentinel", path: "fixture.txt", output_mode: "content" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("Grep empty pattern on a direct file omits the filename", async () => {
		const tool = createAllToolDefinitions(cwd).Grep;
		const result = await tool.execute(
			"call-1",
			{ pattern: "", path: "fixture.txt", output_mode: "content" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("Grep file-list mode names a direct file once", async () => {
		const tool = createAllToolDefinitions(cwd).Grep;
		const result = await tool.execute(
			"call-1",
			{ pattern: "alpha", path: "fixture.txt", output_mode: "files_with_matches" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("Grep direct-file default reports no matching files", async () => {
		const tool = createAllToolDefinitions(cwd).Grep;
		const result = await tool.execute(
			"call-1",
			{ pattern: "absent-sentinel", path: "fixture.txt" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("Grep count mode names a direct file once", async () => {
		...
		const result = await tool.execute(
			"call-1",
			{ pattern: "alpha", path: "fixture.txt", output_mode: "count" },
			undefined,
			undefined,
			{} as never,
		);
